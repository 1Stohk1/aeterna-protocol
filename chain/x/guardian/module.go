...
func (AppModuleBasic) Name() string {
	return guardiantypes.ModuleName
}

func (AppModuleBasic) RegisterLegacyAminoCodec(cdc *codec.LegacyAmino) {
	guardiantypes.RegisterLegacyAminoCodec(cdc)
}

func (AppModuleBasic) RegisterInterfaces(registry codectypes.InterfaceRegistry) {
	guardiantypes.RegisterInterfaces(registry)
}
...
func (am AppModule) RegisterServices(cfg module.Configurator) {
	// Register tx server
	guardiantypes.RegisterMsgServer(cfg.MsgServer(), keeper.NewMsgServerImpl(am.keeper))
	
	// Register query server (which CosmWasm queries via Stargate)
	cfg.QueryServer().RegisterService(&guardiantypes.QueryServiceDesc, keeper.NewQueryServerImpl(am.keeper))
}
